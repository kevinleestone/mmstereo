...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        assert image_width >= self.crop_size[0] and image_height >= self.crop_size[1]
        ...
        if self.generator is not None:
            ...
            for i in range(self.valid_retries):
                if image_width > self.crop_size[0]:
                    left_candidate = self.generator.randint(
                        0, image_width - self.crop_size[0]
                    )
                if image_height > self.crop_size[1]:
                    top_candidate = self.generator.randint(
                        vshift, image_height - self.crop_size[1] - (vshift * 2)
                    )
                area = self.crop_size[0] * self.crop_size[1]
                valid_count = np.count_nonzero(
                    left_disparity[
                        top_candidate : top_candidate + self.crop_size[1],
        ...
        for key, element in sample.elements.items():
            if element.is_image():
                if random_vshift != 0 and key == ElementKeys.RIGHT_RGB:
                    new_data = sample.get_data(key)[
                        top + random_vshift : top + self.crop_size[1] + random_vshift,
                        left : left + self.crop_size[0],
                    ]
                else:
                    new_data = sample.get_data(key)[
                        top : top + self.crop_size[1], left : left + self.crop_size[0]
                    ]
...
class RandomColorJitter(object):
    ...
    def __init__(self, generator):
        ...
        self.jitter = torchvision.transforms.ColorJitter(
            brightness_scale, contrast_scale, saturation_scale, hue_scale
        )
                        ...
                        new_data = np.array(
                            self.jitter(
                                torchvision.transforms.functional.adjust_gamma(
                                    Image.fromarray(sample.get_data(key)), gamma=gamma
                                )
                            )
                        )
                    sample.set_data(key, new_data)
...
class NormalizeImage(object):
    ...
    def __init__(self, normalization_param):
        ...
        self.scale = np.array(normalization_param["scale"], dtype=np.float32).reshape(
            (1, 1, 3)
        )
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        for key, element in sample.elements.items():
            new_data = torch.from_numpy(np.ascontiguousarray(element.data)).to(
                element.tensor_type()
            )
