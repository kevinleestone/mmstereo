...
class Metrics(object):
    ...
    def update_metrics(self, batch, outputs, trainer, key):
        ...
        for output_name, output in outputs.items():
            ...
            if (
                disparity is not None
                and disparity_gt is not None
                and torch.max(disparity_gt) > 0.0
            ):
                disparity_valid_mask = utils.get_disparity_valid_mask(
                    self.hparams, batch_data, output
                )

                self.disparity_metrics(
                    trainer,
                    key,
                    output_name,
                    disparity,
    ...
    def disparity_metrics(
        self, trainer, key, output_name, disparity, disparity_gt, disparity_valid_mask
    ):
        self.error_metric(
            trainer,
            "{}_acc_{}/error".format(key, output_name),
            disparity,
            disparity_gt,
            disparity_valid_mask,
        )
        self.correct_metric(
            trainer,
            "{}_acc_{}/correct_0.25".format(key, output_name),
            disparity,
            disparity_gt,
            disparity_valid_mask,
            0.25,
        ...
        self.correct_metric(
            trainer,
            "{}_acc_{}/correct_0.5".format(key, output_name),
            disparity,
            disparity_gt,
            disparity_valid_mask,
            0.5,
        ...
        self.correct_metric(
            trainer,
            "{}_acc_{}/correct_1.0".format(key, output_name),
            disparity,
            disparity_gt,
            disparity_valid_mask,
            1.0,
    ...
    def correct_metric(
        self, trainer, key, disparity, disparity_gt, disparity_valid_mask, threshold
    ):
        if key not in self.metrics:
