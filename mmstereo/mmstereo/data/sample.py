...
class Sample(namedtuple("Sample", ["metadata", "elements"])):
    ...
    def has_disparity(self):
        return (
            ElementKeys.LEFT_DISPARITY in self.elements
            or ElementKeys.RIGHT_DISPARITY in self.elements
        )

    def is_flippable(self):
        return not self.is_stereo or (
            not self.has_disparity()
            or (
                ElementKeys.LEFT_DISPARITY in self.elements
                and ElementKeys.RIGHT_DISPARITY in self.elements
            )
        )

    def is_stereo(self):
        return (
            ElementKeys.LEFT_RGB in self.elements
            and ElementKeys.RIGHT_RGB in self.elements
        )
    ...
    def swap_left_right(self):
        ...
        def swap(key_left, key_right):
            if key_left in self.elements and key_right in self.elements:
                self.elements[key_left], self.elements[key_right] = (
                    self.elements[key_right],
                    self.elements[key_left],
                )
