...
def make_cv_normalized_disparity_image(disparity, disparity_gt, max_disparity):
    ...
    try:
        min_disparity = torch.clamp(
            torch.min(disparity_gt[disparity_gt > 1e-3]) - 0.1, min=1e-3
        )
        max_disparity = torch.clamp(
            torch.max(disparity_gt[disparity_gt > 1e-3]) + 0.1, min=1e-3
        )
    except RuntimeError:
        min_disparity = 0.0
...
def make_disparity_image(hparams: TrainingConfig, batch_data, output, idx=0):
    ...
    disparity = (
        output["disparity"][idx : idx + 1, :, :, :].clone().detach().to(torch.float32)
    )
...
def make_normalized_disparity_image(hparams: TrainingConfig, batch_data, output, idx=0):
    ...
    if disparity_gt is not None:
        ...
        valid_mask = utils.get_disparity_valid_mask(hparams, batch_data, output)[
            idx : idx + 1, :, :, :
        ]
        disparity_gt[torch.logical_not(valid_mask)] = 0.0
    else:
        disparity_gt = disparity
    mapped = make_cv_normalized_disparity_image(
        disparity[idx, 0, :, :], disparity_gt[idx, 0, :, :], max_disparity
    )
    mapped = cv2.cvtColor(mapped, cv2.COLOR_BGR2RGB)
    mapped = torch.from_numpy(mapped).permute(2, 0, 1)
    return mapped
...
def make_normalized_disparity_gt_image(
    hparams: TrainingConfig, batch_data, output, idx=0
):
    """Generate a disparity visualization as a tensor using OpenCV's jet color mapping"""
    disparity_gt = utils.get_disparity_gt(hparams, batch_data, output)[
        idx : idx + 1, :, :, :
    ...
    disparity_gt[torch.logical_not(valid_mask)] = 0.0
    mapped = make_cv_normalized_disparity_image(
        disparity_gt[idx, 0, :, :], disparity_gt[idx, 0, :, :], max_disparity
    )
    mapped = cv2.cvtColor(mapped, cv2.COLOR_BGR2RGB)
    mapped = torch.from_numpy(mapped).permute(2, 0, 1)
    return mapped
...
def log_disparity(
    ...
    if disparity is not None:
        ...
        tensorboard.add_image(
            "{}{}/disparity_{}/output".format(prefix, suffix, output_name),
            disp_image,
            step,
        )
        ...
        tensorboard.add_image(
            "{}{}/disparity_{}/gt".format(prefix, suffix, output_name),
            disparity_gt_vis,
            step,
        )
        ...
        if scale == 1 and "refine" in output_name:
            tensorboard.add_image(
                "{}{}/normalized_disparity_{}/output".format(
                    prefix, suffix, output_name
                ),
                make_normalized_disparity_image(hparams, batch_data, output),
                step,
            )

            if disparity_gt is not None:
                normalized_disparity_gt_vis = make_normalized_disparity_gt_image(
                    hparams, batch_data, output
                )
            else:
                normalized_disparity_gt_vis = make_dummy_image()
            tensorboard.add_image(
                "{}{}/normalized_disparity_{}/gt".format(prefix, suffix, output_name),
                normalized_disparity_gt_vis,
                step,
            )
...
def log_images(all_loggers, hparams: TrainingConfig, step, prefix, batch, all_outputs):
    ...
    logger.add_image(
        "{}/input_left".format(prefix),
        make_bgr_image(batch_data[ElementKeys.LEFT_RGB]),
        step,
    )
    logger.add_image(
        "{}/input_right".format(prefix),
        make_bgr_image(batch_data[ElementKeys.RIGHT_RGB]),
        step,
    )
    ...
    for idx, (output_name, output) in enumerate(all_outputs.items()):
        ...
        log_disparity(
            all_loggers, hparams, step, prefix, suffix, batch_data, output, output_name
        )
        ...
        if matchability is not None:
            logger.add_image(
                "{}{}/matchability_{}".format(prefix, suffix, output_name),
                make_confidence_image(torch.exp(matchability)),
                step,
            )
